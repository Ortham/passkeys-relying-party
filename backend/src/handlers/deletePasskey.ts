...
export async function deletePasskey(
    ...
    assert(
        user.passkeys.size > 1,
        'The user does not have more than one passkey',
    );
    assert(
        user.passkeys.has(credentialId.toString('base64')),
        "The given credential ID is not for one of the user's passkeys",
    );
...
export const lambdaHandler: Handler = async (
    ...
    assert(
        ...
        'The request does not provide a credential ID',
    );
