...
export function assertParsedJsonIsValid(
    value: unknown,
...
): asserts attestationObject is ValidatedAttestationObject {
    assert.strictEqual(
        attestationObject.fmt,
        'none',
        'Assertion format is not none',
    );
    assert.strictEqual(
        attestationObject.attStmt.size,
        0,
        'Assertion statement is not empty',
    );
...
export function createPasskeyObject(
    requestBody: Omit<RequestBody, 'clientData' | 'description'>,
    user: Pick<User, 'id' | 'userHandle'>,
    ...
    return {
        ...
        uvInitialized: isBitFlagSet(
            requestBody.attestationObject.flags,
            FLAG_USER_VERIFIED,
        ),
        transports: requestBody.transports,
        backupEligible: isBitFlagSet(
            requestBody.attestationObject.flags,
            FLAG_BACKUP_ELIGIBILITY,
        ),
        backupState: isBitFlagSet(
            requestBody.attestationObject.flags,
            FLAG_BACKUP_STATE,
        ),
        description,
...
export async function validatePasskeyInputs(
    passkey: Omit<RequestBody, 'description' | 'transports'>,
    sessionId: string,
): Promise<JsonWebKey> {
    ...
    const passkeyExists = await database.passkeyExists(
        passkey.attestationObject.credentialId,
    );
