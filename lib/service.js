...
export function isValidSessionId(sessionId) {
    return database.getSession(sessionId) !== undefined;
}

export function createSession() {
    const sessionId = getRandomBytes(16).toString('base64url');

    database.insertSession(sessionId);
...
export function createChallenge(sessionId) {
    ...
    database.updateSessionChallenge(sessionId, challenge);
...
export function getProfile(sessionId) {
    const user = database.getUserBySessionId(sessionId);
...
export async function handleSignUp(bodyBuffer, sessionId) {
    ...
    database.updateSessionUserId(sessionId, user.id);
}
...
export async function handleSignIn(bodyBuffer, sessionId) {
    ...
    if (isValid) {
        ...
        database.updateSessionUserId(sessionId, body.userHandle);
    } else {
